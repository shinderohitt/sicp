...
;; tested for equality using equal? (called by assoc). This is not always ;;
;; the appropriate test. For instance, we might have a table with numeric ;;
...
;; constructor make-table that takes as an argument a same-key? procedure ;;
...
(define (same-key? tolerance)
  (define (eq-nums? a b)
    (or (eq? a b)
        (> tolerance
           (abs (- a b)))))
  (lambda (a b)
    (cond ((integer? a) (eq-nums? a b))
          (else (equal? a b)))))
