...
;; to a procedure are evaluated can make a difference to the              ;;
;; result. Define a simple procedure f such that evaluating (+ (f 0) (f   ;;
;; 1)) will return 0 if the arguments to + are evaluated from left to     ;;
...
(define f
  (let ((called-before #f))
    (lambda (arg)
      (if called-before
          0                            ;; has been called before
          (begin
            (set! called-before #t)    ;; called for the first time
            arg)))))
